...
export const login = async (
  req: Request,
  res: Response
): Promise<Response<LoginResponse | CommonError>> => {
...
export const register = async (
  req: Request,
  res: Response
): Promise<Response<RegisterResponse | CommonError>> => {
  ...
  try {
    ...
    if (foundUser)
      return res
        .status(400)
        .json({ error: { message: "User already exists." } } as CommonError);
    ...
    if (!Validator.isValidEmail(email))
      return res
        .status(400)
        .json({ error: { message: errors.INVALID_EMAIL } } as CommonError);
...
export const logout = async (
  req: Request,
  res: Response
): Promise<Response<LogoutResponse | CommonError>> => {
  // TODO: Implement
...
export const getUser = async (
  req: Request,
  res: Response
): Promise<Response<GetUserResponse | CommonError>> => {
  const { id } = req.params;
  ...
  try {
    ...
    if (!id)
      return res
        .status(400)
        .json({ error: { message: "No user data provided." } } as CommonError);
